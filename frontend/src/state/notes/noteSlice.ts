...
const noteSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Handle the toggleArchiveNote async thunk
      .addCase(toggleArchiveNote.pending, (state) => {
        // Optionally handle loading state
        console.log("toggling archive notes...")
        state.status = "loading" // Set status to loading
      })
...
export const { setNoteToEdit, cancelEdit } = noteSlice.actions
export const selectNotes = (state: RootState) => state.notesState.notes
export const selectNoteToEdit = (state: RootState) =>
  state.notesState.noteToEdit
...
export const fetchNotes = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      ...
      return thunkAPI.rejectWithValue(
        "An unknown error occurred in fetching notes"
      )
    }
  }
)
...
export const deleteNote = createAsyncThunk(
  ...
  async (noteId: Note["id"], thunkAPI) => {
    try {
      ...
      return thunkAPI.rejectWithValue(
        "An unknown error occurred in deleting note"
      )
    }
  }
)
...
Note[], // Return type: updated list of notes
Note["id"], // Argument type: ID of the note to toggle
{ state: RootState; rejectValue: string } // ThunkAPI configuration
>(
  "notes/toggleArchiveNote",
  async (noteId, thunkAPI) => {
    const state = thunkAPI.getState()
    const note = state.notesState.notes.find((n) => n.id === noteId)
    if (!note) {
      return thunkAPI.rejectWithValue("Note not found.")
    }
    // Toggle the archived state of the note
    const updatedNote = { ...note, archived: !note.archived }
    //convert tags to comma-separated string and then to filtered array
    const tagsInput = updatedNote.tags ? updatedNote.tags.map((tag) => tag.name).join(", ") : "" 
    ...
    const apiPayload = {
      ...
      tags: tags,
      archived: updatedNote.archived  // Backend expects array of tag names
    }
